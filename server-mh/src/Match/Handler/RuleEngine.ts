...
import { ActiveSafeCellAction } from "./ActiveSafeCellAction";
import { ActivatePenaltyCellAction } from "./ActivePenaltyCellAction";
...
export class RuleEngine {
    ...
    private CheckForSpecialActions() {
    ...
    private CheckForMoveAction(): void {
        ...
        for (const piece of this.player.pieces) {
            ...
            if (destination.isFinal) {
                let reachedFinished: boolean = false;
                let matchFinished: boolean = false;
                if (this.checkForReachedFinished() == 2) {
                    reachedFinished = true;
                    if (this.matchState.winnerList.length == 2)
                        matchFinished = true;
                }
                ...
                this.AddMoveAction(
                    piece,
                    destination,
                    new ActionResult(null, false, false, false, reachedFinished, matchFinished)
                );
            ...
            if (destination.isPenalty) {
                ...
                this.AddMoveAction(
                    piece,
                    destination,
                    new ActionResult(null, true, true)
                );
    ...
    private CellIsEmpty(cell: number): boolean {
        return !this.matchState.players.some(player =>
            player.pieces.some(piece => piece.currentCell === this.matchState.board.cells[cell])
        );
    }
    ...
    private checkForReachedFinished(): number {
        if (!this.player)
            return -1;
